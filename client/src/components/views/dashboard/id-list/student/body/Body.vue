<template>
        ...
        >
          ...
          <template v-slot:[`item.actions`]="{ item }" >
            <div class="flex gap-2">
              ...
              <v-btn
              v-if="idListStore.status === 'issued'"
                @click="
              ...
              <v-btn
              size="small"
              v-if="idListStore.status === 'non-issued'"
                @click="
                
                  idListStore.toggleAprroved({
                    id: item._id,
                    isModalOpen: true,
                  })
                "
  ...
  if (idListStore.status === 'issued') {
    return [...idListStore.issuedStudent].reverse();
  } else {

    return [...idListStore.notIssuedStudent].reverse()
  }
});
console.log(filteredStudentsTable.value);
  const headers = [
